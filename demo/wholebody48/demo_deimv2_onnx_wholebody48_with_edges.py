...
EDGES = [
    ...
    (22, 26), (22, 26),  # shoulder -> elbow (left and right)
    (26, 29), (26, 29),  # elbow -> wrist (left and right)
    (29, 32), (29, 32),  # wrist -> hand (left and right)
    (22, 36), (22, 36),  # shoulder -> hip_joint (left and right)
    (25, 35),            # solar_plexus -> abdomen
    (35, 36), (35, 36),  # abdomen -> hip_joint (left and right)
    (36, 37), (36, 37),  # hip_joint -> knee (left and right)
    (37, 38), (37, 38),  # knee -> ankle (left and right)
    (38, 39), (38, 39),  # ankle -> foot (left and right)
]
...
SIDE_PARENT_TO_CHILDREN = {
    22: (23, 24),  # shoulder -> shoulder_left / shoulder_right
    26: (27, 28),  # elbow -> elbow_left / elbow_right
...
class DEIMv2(AbstractModel):
    def __init__(
        ...
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'deimv2_dinov3_x_wholebody48_960query_n_batch.onnx',
        obj_class_score_th: Optional[float] = 0.35,
        attr_class_score_th: Optional[float] = 0.70,
    ...
    def _postprocess(
        ...
        if len(boxes) > 0:
            ...
            if len(boxes_keep) > 0:
                ...
                # Left and right side attributes merge
                # classid: 22 -> Shoulder
                #   classid: 23 -> Shoulder-Left
                #   classid: 24 -> Shoulder-Right
                # classid: 26 -> Elbow
                #   classid: 27 -> Elbow-Left
                #   classid: 28 -> Elbow-Right
                # classid: 29 -> Wrist
                #   classid: 30 -> Wrist-Left
                #   classid: 31 -> Wrist-Right
                # classid: 32 -> Hand
                #   classid: 33 -> Hand-Left
                #   classid: 34 -> Hand-Right
                # 1. Calculate IoUs for base class results
                # 2. Connect either the Left or the Right class with the highest score and IoU
                # 3. Exclude side attribute classes from detection results
                if not disable_left_and_right_hand_identification_mode:
                    for parent_classid, child_classids in SIDE_PARENT_TO_CHILDREN.items():
                        ...
                        self._find_most_relevant_obj(base_objs=parent_boxes, target_objs=side_boxes)
                ...
                # classid: 22 -> shoulder
                # classid: 23 -> shoulder_left
                # classid: 24 -> shoulder_right
                # classid: 25 -> solar_plexus
                # classid: 26 -> elbow
                # classid: 27 -> elbow_left
                # classid: 28 -> elbow_right
                # classid: 29 -> wrist
                # classid: 30 -> wrist_left
                # classid: 31 -> wrist_right
                # classid: 35 -> abdomen
                # classid: 36 -> hip_joint
                # classid: 37 -> knee
                # classid: 38 -> ankle
...
def draw_skeleton(
    ...
    # -------------------------------------------------
    keypoint_ids = SKELETON_KEYPOINT_IDS
    for box in boxes:
    ...
    for (pid, cid), repeat_count in edge_counts.items():
        ...
        # EDGES に同じ親子を複数回定義した場合は、その回数だけ同一親に接続を許可する
        for_parent = repeat_count
...
def main():
    ...
    parser.add_argument(
        ...
        '--model',
        type=str,
        default='deimv2_dinov3_x_wholebody48_960query_n_batch.onnx',
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=\
            'Disable left and right side attribute identification mode. (Press H on the keyboard to switch modes)',
    )
    parser.add_argument(
        '-dhm',
