...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        if len(dec_out_logits) > 1:
            ...
            return dec_out_bboxes[0], dec_out_logits[0]
...
class RTDETRTransformerv2(nn.Module):
    ...
    def _select_topk(self, memory: torch.Tensor, outputs_logits: torch.Tensor, outputs_coords_unact: torch.Tensor, topk: int):
        ...
        t1 = torch.ones(topk_ind.shape[0], 1, outputs_coords_unact.shape[-1], dtype=torch.int64, device=outputs_coords_unact.device)
        ...
        topk_coords = (outputs_coords_unact * t1).gather(dim=1, index=topk_ind * t1)
