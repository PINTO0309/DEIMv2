...
### `Per-class center-point metrics` の意味

- validation ログに出る `Per-class center-point metrics` は、COCO の `mAP` とは別に、各関節クラスの bbox 中心点が GT 中心点にどれだけ一致しているかを見る補助指標。
- 対象クラスごとに、予測 bbox の中心 `(x1+x2)/2, (y1+y2)/2` と GT bbox の中心を比較する。
- 同一クラス同士だけを score 降順で 1 対 1 に対応付けし、未対応の GT は `FN`、未対応の予測は `FP` として数える。
- 一致判定には正規化中心距離を使う。実装は [`engine/data/dataset/coco_eval.py`](/home/b920405/git/DEIMv2/engine/data/dataset/coco_eval.py#L158) を参照。
...
- `MedD`
  - Median Distance。
  - 正しくマッチしたペアだけを対象にした、正規化中心距離の中央値。
  - 低いほど良い。

#### 正規化中心距離

- 距離は単純なピクセル距離ではなく、次で正規化する。
- `distance / (0.5 * sqrt(w^2 + h^2))`
- ここで `w, h` は GT bbox の幅と高さ。
- つまり GT bbox の半対角長を 1.0 とした相対距離になっている。

#### 閾値の意味

- `F1 / P / R` は、表示上は `@0.5` が省略されているが、実際には `center_eval_primary_threshold` で計算された値。
- 既定値は `0.5`。
- したがって、正規化中心距離が `0.5` 以下で、かつ同一クラスに 1 対 1 で対応付いた予測が `TP` になる。

#### 読み方の目安

- `P` が低く `R` が高い:
  - 位置がそこそこ合う予測は出せているが、過検出が多い。

- `P` が高く `R` が低い:
  - 予測は慎重だが、見逃しが多い。

- `MedD` が低いのに `F1` が低い:
  - 当たった予測の位置精度は悪くないが、`FP` または `FN` が多い。

- `MedD` も高い:
  - そもそも中心位置の回帰がずれている可能性が高い。
